...
- **Strict Workspace Isolation**: Zero files or directories within the `workspace/` directory (including `workspace/goldeneye`) have been modified, refactored, or evolved.
- **Performance Bottleneck Identification**: The core engine bottleneck (e.g., iterative directory discovery latency or inefficient log parsing) is explicitly identified and documented in the agent's reasoning.
- **Targeted Research Executed**: The `web_search` tool is utilized to find a technical solution for the identified bottleneck and to discover a new skill/component pattern.
- **Vault Expansion**: A new component or skill pattern is successfully formulated and saved to the system vault (`library/` or `skills/`).
- **Git Branch Creation**: A new git branch strictly named `evolution/cycle-3` is created and actively checked out.
- **Core Engine Self-Patch**: `bin/atlas_core.py` is successfully updated with concrete logic improvements directly addressing the "Map-First Protocol" or "Traceback-First Diagnostic Protocol" to reduce latency and execution turns.
- **System Verification**: The `verify_project` tool (or equivalent syntax/compilation check) is executed on the core engine, confirming `bin/atlas_core.py` compiles without errors.
- **Push and Notify**: The `evolution/cycle-3` branch is successfully pushed to the remote repository, and the `notify_telegram` tool is executed to request a merge review from the Lead.
